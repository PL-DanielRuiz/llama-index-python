  ...
  const sources: NodeInfo[] = useMemo(() => {
    ...
    data.nodes
      ...
      .forEach((node) => {
        ...
        const nodeInfo = {
          id: node.id,
          url: node.url ?? (fileName ? `/api/files/data/${fileName}` : null), // Usa node.url o node.metadata.file_name si node.url es nulo   // url: node.url,
        };
...
function NodeInfo({ nodeInfo }: { nodeInfo: NodeInfo }) {
  ...
  if (nodeInfo.url) {
    // this is a node generated by the web loader or file loader,SourceNode
